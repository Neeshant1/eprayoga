...
admin.CartDetailsPageViewAdmin= Backbone.View.extend({
    ...
    if(self.cartDetailCollection.length > 0 ){
      ...
      //  $('.add').bind('click', self.add);
        //  $('#minus').bind('click', self.minus);
      });
    }else {
        $('#cartAdmin').hide();
        $('#placeOrderCart').hide();
        $('#backShop').hide();
        $('#cartHide').append('<center><img class="img-responsive" src="js/img/Empty Cart.png" alt="logo"><h2>Your Shopping Cart is empty</h2></center>');
    }
...
add : function(e){
    ...
    // var id = $(e.target).attr('iid');
    // var quant = $('#a'+id).find('#quantity1').val();
    // var value = parseInt(quant);
    
    // value = value+1;
    // if(value>0)
    //   $('#a'+id).find('#quantity1').val(value);
...
placeOrderCart : function(){
  ...
  admin.AdminAppRouter.data = data11;
	appRouter.navigate("performaPageCart", {trigger: true});
},
