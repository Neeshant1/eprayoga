...
exports.BinaryPack = {
  ...
  pack: function(data){
    ...
    packer.pack(data);
    console.log('packer.bufferBuilder():', packer.bufferBuilder);
...
Unpacker.prototype.unpack = function(){
  ...
  console.log('TYPE - unpack_uint8: ', type);
  if (type < 0x80){ //128
    console.log('type < 0x80', type < 0x80);
    var positive_fixnum = type;
    return positive_fixnum;
  } else if ((type ^ 0xe0) < 0x20){
    console.log('(type ^ 0xe0) - 0x20', (type ^ 0xe0) - 0x20);
    var negative_fixnum = (type ^ 0xe0) - 0x20;
    return negative_fixnum;
  }
  var size;
  if ((size = type ^ 0xa0) <= 0x0f){
    console.log('SIZE: raw - type ^ 0xa0: ', type ^ 0xa0);
    return this.unpack_raw(size);
  } else if ((size = type ^ 0xb0) <= 0x0f){
    console.log('SIZE: string - type ^ 0xb0: ', type ^ 0xb0);
    return this.unpack_string(size);
  } else if ((size = type ^ 0x90) <= 0x0f){
    console.log('SIZE: array - type ^ 0x90', type ^ 0x90);
    return this.unpack_array(size);
  } else if ((size = type ^ 0x80) <= 0x0f){
    console.log('SIZE: map - type ^ 0x80', type ^ 0x80);
...
Unpacker.prototype.unpack_uint8 = function(){
  ...
  if (this.index==0) console.log('byte: ', byte);
  this.index++;
  return byte;
};
...
Packer.prototype.pack_string = function(str){
  console.log('meta string: ', str);
  var length = utf8Length(str);
  console.log('meta string utf8Length: ', length);
  if (length <= 0x0f){ //0x0f - 15
    this.pack_uint8(0xb0 + length); //0xb0 - 176
  } else if (length <= 0xffff){ //0xffff - 65535
    this.bufferBuilder.append(0xd8) ; //216
    this.pack_uint16(length);
  } else if (length <= 0xffffffff){
...
Packer.prototype.pack_array = function(ary){
  ...
  if (length <= 0x0f){  //0x0f=15
    console.log('unit8');
    console.log('0x90 + length: ', 0x90 + length); //prints 144+3=147
    this.pack_uint8(0x90 + length); //0x90=144
  } else if (length <= 0xffff){ //0xffff=65535
    console.log('unit16');
    this.bufferBuilder.append(0xdc) //0xdc=220
    this.pack_uint16(length);
  } else if (length <= 0xffffffff){ //0xffffffff=4294967295
    console.log('unit32');
    this.bufferBuilder.append(0xdd); //0xdd=221
    this.pack_uint32(length);
  } else{
    throw new Error('Invalid length');
  }
...
Packer.prototype.pack_integer = function(num){
  if ( -0x20 <= num && num <= 0x7f){ //-0x20=-32  0x7f=127
    this.bufferBuilder.append(num & 0xff);//0xff=255
    console.log('num & 0xff', num & 0xff); //this prints 1
  } else if (0x00 <= num && num <= 0xff){ //0xff=255
    this.bufferBuilder.append(0xcc); //0xcc=204
    this.pack_uint8(num);
...
Packer.prototype.pack_object = function(obj){
  console.log('packing object...');
  var keys = Object.keys(obj);
  console.log('keys: ', keys);
  var length = keys.length;
  if (length <= 0x0f){
    this.pack_uint8(0x80 + length); //0x80=128 + 2
    console.log('0x80 + length', 0x80 + length);//130
  } else if (length <= 0xffff){
  ...
  console.log('this.bufferBuilder: ', this.bufferBuilder);
  for(var prop in obj){
...
exports.EventEmitter = EventEmitter; //NP Added By Vahai to use it in WS Support
...
var util = {
  ...
  setZeroTimeout: (function(global) {
    ...
    if (global.addEventListener) { //which doesn't work with IE
      global.addEventListener('message', handleMessage, true);
    } else if (global.attachEvent) { //work with IE
...
Stream.prototype.pipe = function(dest, options) {
  ...
  function ondata(chunk) {
    if (dest.writable) {
      console.log('this.noOfChunks - ', ++this.noOfChunks);
...
BlobReadStream.prototype._emitReadChunk = function(){
  ...
  console.log('before _read');
  this._read();
  console.log('after _read');
...
function BinaryStream(socket, id, create, meta) {
  ...
  if(create) {
    ...
    console.log('creating new stream & sending meta to server!!!');
...
function BinaryClient(socket, options) {
  ...
  if(typeof socket === 'string') {
    ...
    console.log('Socket State1: ', this._socket.readyState);
  } else {
  ...
  this._socket.addEventListener('message', function(data, flags){
    console.log('data1 From SERVER: ', data);
    util.setZeroTimeout(function(){      
      // Message format
      // [type, payload, bonus ]
      //
      ...
      data = data.data;
      console.log('data2 From SERVER: ', data);
      console.log('data..byteLength: ', data.byteLength);
      try {
          data = util.unpack(data);
          console.log('unpack data From SERVER:', data);
      } catch (ex) {
          console.log("Received unparsable message: ", ex);
...
BinaryClient.prototype.send = function(data, meta){
  ...
  if(data instanceof Stream) {
    ...
    if(data.constructor == Blob || data.constructor == File) {
      ...
      console.log('TypeOf data is Nothing/Default');
      // stream.write(data);
    }
  }
  return stream;
...
BinaryClient.prototype.createStream = function(meta){
  console.log('Socket State2: ', this._socket.readyState);
