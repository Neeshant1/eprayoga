...
console.log("self in js=", self);
var gadgets;

function makeGadgets() {
  if (!gadgets) {
    if (!this.JSON) {
      this.JSON = {}
    }(function() {
    ...
    gadgets = {
      ...
      TabSet: function(id) {
      	console.log('-------TabSet called');
        var me = this;
        console.log('this inside TabSet', this);
        this.tabs = [];
        ...
        this.addTab = function(id, parms) {
          ...
          console.log(oTab);
          console.log(parms.contentContainer);
    ...
    console.log('gadget created', gadgets);
  }
  ...
  var puzzleObj = initAll();
  return puzzleObj;
}

function createPuzzleInstance() {
// 	$("*").each(function(i,e){console.log(i+' = ');
// console.log(e)});
console.log('inside createPuzzleInstance');
//   var tabItems = ["Solve", "Create"];
...
//     var id = tabItems[i];
//     console.log('id----->'+id);
//     console.log(document.getElementById("puzzleTab" + id));
//     console.log($('#puzzleTab'));
//     console.log('going to call addTab from initAll');
//     self.puzzleGadgetTabs.addTab(id, {
//       contentContainer: self.document.getElementById("puzzleTab" + id)
//     });
//   }
//   console.log('finished loop!!!!!!!!!!!!!!!!!!!!!')
  ...
  console.log('creating new puzzle in initAll');
  return new Puzzle("puzzleParent", options);
  // console.log('new puzzle creation in initAll finished');
}
...
function Bbox(a, b, c, d) {
	// console.log('************ Bbox creation====', a,b,c,);
...
Bbox.prototype.pointIn = function(p) {
	// console.log('--- pointin of Bbox');
	// console.log('tl.x=',this.tl.x, p.x);
	// console.log('br.x=',this.br.x, p.x);
	// console.log('tl.y=',this.tl.y, p.y);
	// console.log('br.y=',this.br.y, p.y);
...
PuzzleTransientTile.prototype.pointIn = function(p) {
	// console.log('--- pointin of PuzzleTransientTile');
  if (!this.tImage) {
    this.sync();
  }
...
PuzzleTransientTile.prototype.merge = function(others) {
	console.log('merging transient tile');
...
PuzzleDisplayTile.prototype.pointIn = function(p) {
	// console.log('--- pointin of PuzzleDisplayTile');
...
PuzzleDisplayTile.prototype.merge = function(others) {
	console.log('merge PuzzleDisplayTile');
...
PuzzlePiece.prototype.pointIn = function(p) {
	// console.log('--- pointin of PuzzlePiece');
...
PuzzlePreview.prototype.pointIn = function(p) {
	// console.log('--- pointin of PuzzlePreview');
...
PuzzleBed.prototype.pointIn = function(p) {
	console.log('--- pointin of PuzzleBed');
  return false;
};
 ...
 *   Use Default
 */
function Puzzle(el, puzzleOptions) {
  // this is me initializing
  console.log('---------------- inside puzzle function', this);
  var me = this;
  console.log('me=',me);
  // Methods
  ...
  this.create = function(puzzleOptions) {
    ...
    // !!! Not used cookies. directly storing in db
    // if (!puzzleOptions) {
    ...
    this.config.src = puzzleOptions.src;
    this.config.imgObjURL = puzzleOptions.imgObjURL;
    ...
    if (this.config.imgObjURL)
    {
    	this.imageSource.src = this.config.imgObjURL;
    }
    else
    {
    	this.imageSource.src = URL.createObjectURL(this.config.src);
    }
  ...
  console.log('passed create fun assign');
  // mix pieces
  ...
  this.partUnderPoint = function(p) {
  	// console.log('checking part under point-----------');
  ...
  this.getConfig = function(noProgress) {
    var opts = this.config;
    // var attributes = {};
    ...
    if (!noProgress && this.composites) {
      for (var pid in this.composites) {
        ...
        clusters.push(this.composites[pid].composite);
      }
    }
  ...
  this.syncUI = function() {
    ...
    var opts = this.config;
    console.log('in sync=',this.config);
    // Set the values to front end
    // this.puzzleShowEdges.value=opts.showEdges?"Show all pieces":"Show edge pieces only";
    this.puzzleShowPreview.value = opts.showPreview ? "Hide Original" : "Show Original";
  ...
  for (var iElem = 0; iElem < elems.length; iElem++) {
    ...
    // 		this[e.id] = URL.createObjectURL(e.files[0]);
    // }
    // else
    // {
    // 	this[e.id] = e;
  ...
  this.puzzleCreate['createPuzzle'] = function() {
  	console.log('me in button press', me);
    // alert(me.imageupload.files.item(0));
  ...
  this.puzzleKeyInCreate['createPuzzleFromKey'] = function() {
    ...
    if (input.length > 0) {
      ...
      me.create(config);
      self.puzzleGadgetTabs.setSelectedTab(0);
      console.log('inside keyincreate');
      me.syncUI();
      me.markAsDirty();
    }
  };
...
// commented to prevent creating puzzle by default
 // console.log('calling puzzle.create when new Puzzle');
  // this.create(puzzleOptions);
  // console.log('also calling sync');
  // this.syncUI();
 // !!!!!!!!!!!
